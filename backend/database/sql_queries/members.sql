...
SELECT member_id, user_id, weight, height, fitness_goal, fitness_level, health_conditions, active_cycle_id
FROM members
WHERE user_id = %s;
...
-- NAME: create
INSERT INTO members (user_id, weight, height, fitness_goal, fitness_level, health_conditions, active_cycle_id)
VALUES (%(user_id)s, %(weight)s, %(height)s, %(fitness_goal)s, %(fitness_level)s, %(health_conditions)s, %(active_cycle_id)s);
